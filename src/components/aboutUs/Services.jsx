...
const defaultServicesData = {
  heading: "Our Services",
  items: [
    { name: "Haircut", description: "Professional haircut and styling tailored to your look.", icon: "✂️" },
    { name: "Facial", description: "Relaxing facial treatments to rejuvenate your skin.", icon: "💆‍♀️" },
    { name: "Spa", description: "Premium spa services to refresh your body and mind.", icon: "🛀" },
    { name: "Manicure", description: "Pamper your hands with professional manicure services.", icon: "💅" }
  ]
};

const Services = ({ data, activeService, setActiveService }) => {
  const servicesData = data || defaultServicesData;
      ...
      <div className="container mx-auto">
        ...
        <div className="flex justify-center gap-6 flex-wrap">
          {servicesData.items.map((service, idx) => (
            ...
            >
              <div
                className={`text-4xl mb-4 transition-colors duration-400 ${
                  activeService === idx ? "text-white" : "text-[#164374] group-hover:text-white"
                }`}
              >
                {service.icon}
              </div>
              <h3
                className={`text-xl font-semibold mb-2 ${
                  activeService !== idx && "group-hover:text-white"
                }`}
              >
                {service.name}
              </h3>
              <p
                className={`text-sm leading-normal transition-colors duration-400 ${
                  activeService === idx ? "text-white/90" : "text-[#6c757d] group-hover:text-white/90"
                }`}
              >
                {service.description}
              </p>
