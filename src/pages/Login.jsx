...
const Login = () => {
    ...
    const verifyOtp = async (k) => {
        ...
        if (verifyOtpResponse.ok) {
            ...
            // The backend response should now include `isProfileUpdated`
            // Example:
            // {
            //   "mobile": "9999648406",
            //   "status": "success",
            ...
            if (verifyOtpRespo.status === "success") {
                if (verifyOtpRespo.authToken !== null) {
                    authCtx.login(
                        verifyOtpRespo.authToken,
                        verifyOtpRespo.mobile,
                        verifyOtpRespo.isProfileUpdated // Pass the new value to the context
                    );
                    nav(`/`); // Navigate away, the Layout component will handle showing the popup
                } else {
                    authCtx.logout();
                }
            } else {
	...
	// The rest of your Login component's JSX remains the same
    return (
		<div className="px-5 h-[80vh] flex justify-center items-center font-inter bg-white">
			<div className="flex justify-center items-center w-full animate-fadeIn">
				<div className="bg-white border border-black/5 rounded-lg p-8 w-full max-w-md text-center shadow-md animate-slideUp">
					{step === 1 && (
						<div>
                        <h2 className="mb-3 text-2xl font-bold text-black flex items-center justify-center">
...
>
  <div className="flex gap-3 mb-5 p-2 rounded-xl border border-gray-100">
    <select
      className="flex-[0.3] px-3 py-2 rounded-lg bg-gray-100 text-black font-semibold text-sm cursor-pointer focus:outline-black"
      value={countryCode}
    ...
    <input
      ref={phoneInputRef}
      type="text"
      className="flex-1 px-3 py-2 bg-gray-100 rounded-lg border-none focus:outline-black"
      ...
      <span className="flex items-center justify-center">
        Next <FaPhoneAlt className="ml-2" />
                    ...
                    >
                        ...
                        <MdOutlinePassword className="mr-2" /> We have sent a 6 digit OTP to {phone} on WhatsApp
                        </div>

                        <h2 className="text-xl font-bold text-black mb-4">Enter OTP</h2>

                        <CustomOTPInput
  length={6}
  onChange={(otpStr) => {
    setOtp(otpStr);
    if (otpStr.length === 6) verifyOtp(otpStr);
  }}
  ref={otpInputRef}
/>
                        ...
                        <button
                        type="submit"
                        className="w-full mt-4 bg-black text-white font-semibold py-3 rounded-lg shadow-md hover:shadow-lg hover:-translate-y-0.5 transition"
                        >
                        Submit OTP
                        </button>
                        ...
                        type="button"
                        onClick={() => setStep(1)}
                        className="mt-4 text-gray-600 hover:text-gray-600 flex items-center justify-center gap-2 text-sm font-medium"
                        >
                        <IoArrowBack /> Back
                        </button>
                    </form>
...
// 						<div>
// 							<div className="px-4 py-3 text-gray-600 text-sm leading-6 mb-3 flex items-center justify-center">
// 								<MdOutlinePassword className="mr-2" /> We have
// 								sent a 6 digit OTP to {phone} on WhatsApp
// 							</div>
