...
const PriceList = () => {
  ...
  const handleSearchChange = (e) => {
    ...
    const filtered = priceList?.filter(
      (item) =>
        ...
        item.services?.some((s) =>
          s.addOns?.some((a) => a.label?.toLowerCase().includes(lower))
        )
    );
  ...
  return (
    <div className="min-h-screen px-2 bg-white font-poppins">
      {/* Sections Tabs */}
              ...
              {filteredPriceList
                ...
                .map((itemPrice) => (
                  <div
                    ...
                    onClick={() =>
                      scrollToCategory(itemPrice.categoryIdentifier)
                    }
                  >
      ...
      {/* Footer Cart */}
      <div className="sticky bottom-0 bg-white/85 backdrop-blur-md px-6 py-4 border-t border-gray-300/50 flex justify-between items-center rounded-t-xl shadow-[0_-4px_15px_rgba(0,0,0,0.05)] mt-6">
        <div className="flex items-center gap-2 font-medium text-black">
          <FaShoppingCart className="text-lg" />
          <div className="text-sm">{cartItems.length} item(s) in cart</div>
        </div>
        <button
